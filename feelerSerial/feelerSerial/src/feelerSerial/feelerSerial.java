...
public class feelerSerial {
	...
	// Button bools to activate when button is pressed and deactivate when
	...
	// Set settings that are sent to arduino. They are sent only if you set
	// this.
	...
	public void init(String adress) {
		...
		if (!debugMode) {
			...
			while (portBoolean == 0) {
				...
				for (int i = 0; i < temp.length; i++) {
					if (temp[i].equals(adress)) {
						try {
							mySerial = new Serial(myParent, adress, 56700); // connect
																			// to
																			// serial
							portBoolean = 1;
							break;
			...
			while (true) {
				mySerial.write(startMarker + "S" + limitMarker + Integer.toString(box1Speed) + limitMarker
						+ Integer.toString(box2Speed) + limitMarker + Integer.toString(box3Speed) + endMarker);
				System.out.print(".");
			...
			System.out.println("Serial debug mode started");
	}
	...
	// Get the serial information and return it if received successfully, else
	// return NULL
		...
		if (!debugMode) {
			...
			while (mySerial.available() > 0) {
				...
				if (receiving == true) {
					...
					// return if max length of serial is filled
					if (input == endMarker) {
						if (index > maxLengthOfSerial - 1)
							return null;
						receivedSerial[index + 1] = '\0';
						// clear rest
						while (mySerial.available() > 0)
							mySerial.read();
						// copy to new array, and make it the right length
						char[] arr2 = Arrays.copyOf(receivedSerial, index + 2);
		...
		while (index < maxLengthOfSerial) {
			...
			if (parsing == true) {
				...
				if (serialSettings[index] == limitMarker) {
					...
					//System.out.println("boxesConnected: " + Integer.toString(boxesConnected));
					//debug
					boxStateInput = intSettings[1];
					//System.out.println("boxStateInput: " + Integer.toString(boxStateInput));
					//debug
					box3Button = intSettings[2]; 
					//System.out.println("box3Button: " + Integer.toString(box3Button));
					//debug
					//Put the boxbool on according to the information
					if (box3Button != box3ButtonTemp) {
						if (box3Button == 1)
							box3Button1 = true;
						else if (box3Button == 2)
							box3Button2 = true;
						else if (box3Button == 3)
							box3Button3 = true;
					}
					return true;
				}
	...
	// Get the boxState, returns "stopped" or "playing". Or a longer string if
	// program and device is not synced.
	public String getBoxState() {
		if (boxStateInput == 0 && boxState == boxStateInput)
			return "stopped";
		else if (boxStateInput == 1 && boxState == boxStateInput)
			return "playing";
	...
	public void sendValues() {
		String write1 = startMarker + "s" + limitMarker + Integer.toString(boxState) + limitMarker
				+ Integer.toString(box2LedState) + endMarker;
		if (!debugMode) {
	...
	public void debugSet(int one, int two, int three) {
		if (debugMode) {
			debugString = startMarker + "b" + limitMarker + Integer.toString(one) + limitMarker + Integer.toString(two)
					+ limitMarker + Integer.toString(three) + endMarker;
		}
	}
