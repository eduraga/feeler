public void overallAvgs(int i, String position, String caption){
  ...
  if(fileNames[0].charAt(0) != '.'){
    ...
    fill(textDarkColor);
    text("Relaxation "+round(relaxationAverageList[i])+"%", thisX, visY, visWidth/2, visHeight/2);
    
    fill(70);
    rect(thisX + padding, visY + visHeight/2, attAvg, visHeight/2);
    fill(textDarkColor);
    text("Attention "+round(attentionAverageList[i])+"%", thisX, visY + visHeight/2, visWidth/2, visHeight/2);
