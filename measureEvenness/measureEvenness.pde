//will measure and graph the time distances between each sample, to asess sampling interval regularity
import controlP5.*;
ControlP5 cp5;
Chart myChart;
PrintWriter output;
String[] fileLines;
//println("there are " + fileLines.length + " lines");
...
void setup() {
  ...
  output = createWriter("timeUnevenness.txt");
}
int indx=1;
int prevTime=0;
...
void draw() {
  //time distance, assumming that is in milliseconds
  int difference=0;
  background(200);
  if (indx<fileLines.length) {
    String [] dataPieces=split(fileLines[indx], TAB);
    if (dataPieces.length>0) {
      String logString="";
      int currentTime=int(dataPieces[0].replace('.', '0'));
      difference=currentTime-prevTime;
      if (difference>largestDifference) {
        largestDifference=difference;
        myChart.setRange(0, largestDifference);
        println("LARGEST!!!"+largestDifference);
        logString+="!! LARGEST";
      }
      logString+="line {"+dataPieces[0]+"} time "+indx+":"+currentTime+" dif="+difference;
      println(logString);
      myChart.push("incoming", difference);
